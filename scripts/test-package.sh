...
case "$platform" in
  linux-x64)
    docker run --rm --entrypoint /bin/bash -v "$test_dir:/var/task" -w /var/task public.ecr.aws/lambda/nodejs:24 -c 'npm install --ignore-scripts && node smoke.cjs'
    ;;
  linuxmusl-*)
    docker run --rm -v "$test_dir:/app" -w /app node:24-alpine sh -c 'npm install --ignore-scripts && node smoke.cjs'
    ;;
  darwin-arm64v8)
